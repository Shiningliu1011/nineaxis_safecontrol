...
from xy_mid_360_s.ros_node import (  # noqa: E402
    _POINT_DTYPE,
...
from xy_mid_360_s.stream import CloudFrame  # noqa: E402
...
def _points_xyz(msg):
    """Self-contained copy of the originating project's PointCloud2 decoder.

    ``sensor_msgs_py`` refuses mixed-dtype point fields (Livox PointCloud2 has
    float32 xyz + uint8 tag/line), so the raw byte-buffer fallback is the path
    that actually runs for these messages.
    """
    try:
