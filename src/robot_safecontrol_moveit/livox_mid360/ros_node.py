...
protocol/decoding comes from :mod:`~robot_safecontrol_moveit.livox_mid360.protocol`
and :mod:`~robot_safecontrol_moveit.livox_mid360.stream`, and this module is the
...
Usage::
    ...
    # or, from the installed console script:
    ros2 run robot_safecontrol_moveit livox_mid360_node --ros-args \
...
change device state (see ``config/perception_dual_sensor_real.yaml``).  Enable it
only for an intentional bring-up, or configure the device once with
``livox_mid360_tool mode set sampling --ip <addr>``.
"""
...
from robot_safecontrol_moveit.livox_mid360.stream import CloudFrame, CloudReceiver, StateMonitor
    ...
    def __init__(self) -> None:
        ...
        if self._configure_on_start and lidar_ips:
            ...
            self.get_logger().info(
                "listen-only mode: run 'livox_mid360_tool mode set sampling --ip <addr>' "
