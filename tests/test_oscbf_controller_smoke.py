...
def test_publishes_valid_joint_state(controller_fixture):
    node = controller_fixture["node"]
    context = controller_fixture["context"]

    probe = rclpy.create_node(
        "oscbf_controller_smoke_probe", context=context
    )
    received = []
    ...
    probe.create_subscription(
        JointState, _TOPIC, _on_state, qos_profile_sensor_data
    )
    plant_pub = probe.create_publisher(
        JointState, _TOPIC, qos_profile_sensor_data
    )
    ...
    plant = JointState()
    plant.header.stamp = probe.get_clock().now().to_msg()
    ...
    probe.destroy_node()
    assert output is not None, "controller published no safe state"
