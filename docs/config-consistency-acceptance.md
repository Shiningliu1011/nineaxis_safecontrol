...
| 九轴身份、最终话题连接与实际圆柱拟合来源 | PASS | 规范 joint_names 顺序；ROS 语法、raw 及 remap 后连接校验；链式 remap 实体/快照一致；真实 geometry 断言 |
...
最终 HEAD 的真实节点快照记录了：run id、schema/node、最终值、逐项来源、覆盖链、资源原值/解析路径、raw/resolved 话题连接、生产 YAML SHA-256、Git HEAD/dirty/status、46 个控制源文件摘要、实际拟合圆柱中心/轴/半径、障碍 CBF 基准 alpha 10 及 perception 第十槽运行值来源。连续直接入口与 launch 入口生成不同文件，未覆盖既有证据。
