...
单位ms，稳态公开调用至主机结果返回；每个后端的5组均未出现QP失败，源码前后hash一致。

| 场景 | CPU步数 | CPU p95 / p99 / max | GPU步数 | GPU p95 / p99 / max |
|---|---:|---:|---:|---:|
| full_disabled | 1705 | 8.251 / 8.957 / 12.655 | 1723 | 16.819 / 18.765 / 22.536 |
| full_far8 | 1597 | 12.006 / 12.443 / 13.006 | 1598 | 19.461 / 20.196 / 22.274 |
| moving_near8 | 3000 | 12.133 / 12.681 / 14.873 | 3000 | 20.787 / 22.345 / 29.769 |
| far8_unloaded | 3000 | 12.191 / 12.902 / 18.504 | 3000 | 19.475 / 20.226 / 22.595 |
| far8_fusion_load | 3000 | 12.530 / 13.766 / 17.691 | 3000 | 19.978 / 21.377 / 33.203 |

合计24623步；四组full回放全部达到内核completed状态，近障两组均为0进度、最小距离-0.03米，绝不计为避障/任务通过。近障GPU超过20ms占18.867%。两组3000步负载对照包含端点后的内核调用，不能与到端点立即停止的full分布直接等同。每后端负载/无负载最终q完全一致；跨后端的完整等价未验收。

- CPU：初始化含预热18.923s；融合负载343周期/34.337s，全部输出有效，融合p50/p99/max为26.651/33.437/36.241ms。这里的有效是现有融合状态位，不是可信时钟/安全覆盖验收。
- GPU：初始化含预热78.016s；融合负载567周期/56.764s，全部输出有效，融合p50/p99/max为24.937/37.888/41.635ms。这里的有效是现有融合状态位，不是可信时钟/安全覆盖验收。

本轮没有GPU替代收益。CPU组合负载p99相对配对空载约增加6.7%，GPU约增加5.7%；只有一次顺序配对且后台未完全控制，不能作为普适开销比例。全组CPU样本没有超过20ms，仍不构成20ms上界或生产50Hz准入。

## 对齐起点的节点结果

两后端各3000次回调，无保持、无QP失败。CPU/GPU采用同一个IK起点，未绕过任何回调保护逻辑。

| 后端 | 完整回调p95 / p99 / max (ms) | 内含step_once p99 | 内含低通及发布p99 | 最终路径进度 |
|---|---:|---:|---:|---:|
| cpu | 8.527 / 9.239 / 13.584 | 8.932 | 0.337 | 0.234579 m |
| gpu | 17.895 / 19.488 / 42.715 | 19.133 | 0.327 | 0.234579 m |

这补足当前节点回调到发布返回的边界，比只测内核更接近实际软件路径，但仍无定时器排队/DDS/CAN。内含分段的分位数不可相加或相减推导其它阶段。低通反馈下30秒名义步长只推进约0.235米（全路径1.817米），未完成任务。当前节点参数包含solver_tol=0.001、隐式上一命令与低通反馈，与继承首轮参数的内核理想q_next回放属于不同测量对象，禁止将两组差异归因于单一模块。
