...
2026-09-07。状态：决策记录与窗口顺序已通过 PR #28/#29 发布；地图 #1 和所有开放原票已同步，尚未执行本轮算法迁移。工作区原有大量未提交改动，本次发布只提交规划文档，没有把这些改动混入。初轮远端地图快照保存在 previous-remote-map.md，供追溯旧口径。
...
历史测试数量、p95、硬件状态均保留为历史证据，本轮不声称重新验证。远端“选定控制率/延迟预算与实现策略”仍 open；正文已改为将 50Hz/20ms 视作历史工程基线，具体周期、后端和实机预算等待新测量，不重复询问已接受原则或自动关闭。
...
2026-09-07 已重新读取远端地图、原票、评论与状态，并完成正文/同步评论更新。PR #29 合入逐票窗口顺序；地图 #1 只保留目标、全局约束和原票索引，历史实现细节留在旧快照与具体票中。#8、#16–#19、#22、#25 已修正过时方向；全部开放票均保留原编号并记录当前窗口位置。

后续新增票先核对现有 backlog，确有缺口才创建并取得真实 issue 数据库 ID，再加原生 sub-issue 和 dependencies。不得将仓库 research 文档完成冒充远端 issue 已关闭，也不得在具体数值、工况或实测证据不足时关闭 #16–#18 及实施票。
