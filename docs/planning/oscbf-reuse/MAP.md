...
- 本文已从临时目录迁入仓库 docs/planning/oscbf-reuse，是现有 GitHub 地图的改版草案，不是另一份权威 tracker。`gh` 未认证，未发布、未远端领取/关闭任何票；本地文件名不是正式 issue 身份。发布时更新现有地图，复用已有票据，新增票再绑定原生 sub-issue 与 blocking。
...
- 上述优先级已由用户补充为效果择优：成熟开源优先评估，已有实现可凭比较优势保留；替换须有验证与收益依据，详见[维护边界补充决议](issues/03-reuse-boundary.md#用户补充决议按实际效果择优保留待发布)。自碰撞保留 OBB 的阶段决议见[控制接入](issues/05-control-adoption.md#阶段决议自碰撞保留现有-obb)。
...
本轮复用与迁移规划已本地收敛；实施证据与远端发布尚待完成。此前已定的需求从原票读取，不复制答案：
...
- [确定本项目独有能力与上游维护边界](issues/03-reuse-boundary.md)：保留研究插件并接受受控的最小上游补丁维护；用户已确认，本地已解决，待远端发布。
...
- [确定端到端验证门与故障处理边界](issues/06-validation-contract.md#resolution-comment待发布)：任务与故障恢复、工况预算、实机松弛边界和实际执行验收已定；本地已解决，数值证据仍为启用前置。

- [确定渐进迁移顺序与旧实现退役条件](issues/07-migration-order.md#resolution-comment待发布)：分段对照、受控切换、停止后回退及验收后退役已接受；本地已解决。

## Not yet specified

当前规划范围无未归属决策。实施中才能获得的兼容补丁、数值预算和逐文件退役证据，已交接至[迁移决议](issues/07-migration-order.md#规划收束与剩余证据)及其引用的原票；若证据改变架构前提，再重开对应决策。
