...
2026-09-12 更新：用户已要求把能独立处理的离线部分拆成新票。离线执行优先读取 [OFF 队列及执行说明](OFFLINE-EXECUTION-ORDER.md)，不要因为本表原票的整体前置仍开放而阻塞独立离线工作。本表继续记录原票的整体研究、决策及现场验收关系；已拆出的实现只在 OFF 票执行一次。
