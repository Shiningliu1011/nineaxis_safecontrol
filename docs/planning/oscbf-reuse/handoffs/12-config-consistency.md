...
## 2026-09-11 完成审查与 TDD 补验

完成审查发现唯一阻塞：原验收报告将“相同有效配置下的任务误差、路径进给、约束残差与 QP 健康回归”标为通过，但测试只证明当前版本可运行，没有固定起点 `84797408ac0cbe344bcb9f06fc70da5ae29dd417` 与当前实现的同输入对照。其余生产配置、覆盖链、消费者锁定、快照门禁和兼容边界均有直接证据。

用户随后明确要求用 TDD 解决该发现。确认测试 seam 为生产控制器公开的 `OscbfController.step_once`，独立真值来自固定起点 detached worktree，以相同生产哨兵配置和蝴蝶轨迹起点执行四步闭环。红灯先因公开结果缺少路径进度而出现；最小实现仅增加 `path_progress_m` 和 `qp_primal_residual` 两个诊断字段，没有修改控制计算或命令。

绿灯 `test_production_config_refactor_preserves_control_outputs_from_8479740` 对照四步 `err_6d`、`feedrate_m_s`、路径进度、QP primal residual 和 `qp_ok`；期望值为固定起点采集的独立字面量，不由当前实现生成。实现提交 `c3691492a6565c37cc37254747856e5e791a1c66`。相关测试 52 passed；正式总入口复验为主包 347 passed，portable OSCBF 148 passed / 34 skipped，两套及总退出码均为 0；compileall 与 diff check 通过。CUDA-enabled jaxlib 仍缺失并按既有规则回退 CPU，不影响本票结论；真机范围仍由原执行链路票负责。

至此八项配置契约与全部必要验收均已满足，可以发布本票 resolution、关闭原票并向正式地图追加一条上下文指针。本次未调整 CBF 数值、控制周期、真机后端或反馈链。
