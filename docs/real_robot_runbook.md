...
> 2026-09-11 当前入口见[笔记本连接与诊断](hardware_laptop_quickstart.md)。
> 当前 shadow 不读 CAN、不发布硬件状态；live 在 bridge 与 final launch 均拒绝启动。
> 下文为历史验收流程草案，§2.2–§5 不是当前可执行能力，不能按其命令启动运动。
> 请先完成连接、设备身份、标定与独立执行保护，再按受控低速验收票更新本文。
